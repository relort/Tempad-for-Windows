...
-------------------------------------------------------------------------------
    Refactoring and some code-modifications, bug-fixing by Inline-CODER, 2010.

    web-site :   http://inline-coder.net/
    e-mail   :   inline-coder@inbox.ru
...
const
    ...
    CursorArray : array [TGridHitTest] of TCursor = ( crDefault,
                                                      crLeftTop,
                                                      crRight,
                                                      crDown,
                                                      crPlus,
...
begin
  ...
  //
  if AKind = SB_HORZ
      then Info.nPage := Width
      else Info.nPage := Height;
  //
...
begin
  ...
  SetScrollBar(SB_HORZ, FMaxHScroll {*} + Width  {*}, FHorzOffset, SIF_POS or SIF_RANGE or SIF_PAGE);
  SetScrollBar(SB_VERT, FMaxVScroll {*} + Height {*}, FVertOffset, SIF_POS or SIF_RANGE or SIF_PAGE);
...
begin
  ...
  begin
    P := PHeaderInfo(FHeaderInfos[x]);
    R := Rect(
           GetColCoord(P^.Rct.Left) - 1,
           (FDefRowHeight * P^.Rct.Top) - 1,
           GetColCoord(P^.Rct.Right + 1),
           FDefRowHeight * (P^.Rct.Bottom + 1)
         );
...
begin
    ...
    if IntersectRect(Dummy, Rc, CellBox)
        then begin
                 Column := FColumns[x];
...
begin
  ...
  begin
    ...
    begin
      ...
      if (s = LastY.Str) then
      begin
        LastY^.Rct.Bottom := Min(FHeaderLine-1, Max(LastY^.Rct.Bottom, y));
      end else
      begin
        if (s = PHeaderInfo(LastX[y])^.Str) then
        begin
          P := PHeaderInfo(LastX[y]);
          P^.Rct.Right := P^.Rct.Right + 1;
        end else
        begin
          P := New(PHeaderInfo);
          P^.Rct := Rect(x, y, x, y);
...
procedure TNiceGrid.InternalSetCell(X, Y: Integer; Value: string; FireOnChange: Boolean);
var t : TStringList;
    s : string;
    CanChange : Boolean;
begin
...
//                     then FOnCellChange(Self, X, Y, s);
                 ...
                 // moved here in case of invalid event data sent to event handler (old string, not new)
                 if Assigned(FOnCellChange)
                     then FOnCellChange(Self, X, Y, s);
             end
        else t[Y] := Value;
    ...
    if (ssCtrl in Shift)
        then begin
                 case Key of
                     ...
                     VK_DELETE
                         : begin
                               if not FReadOnly and (FRowCount > 1)
                                   then begin
                                            Old := FRow;
                                            DeleteRow(FRow);
                 ...
                 then begin
                          case Key of
                          // inline-coder ленькі дальше рефакторить
      VK_LEFT:
        begin
        ...
        crDown
            : begin
                  if not ( ssDouble in Shift)
                      then begin // left or right click
                               FCol        := GetColFromX(X);
                               FRow        := 0;
                               LastHover   := FCol;
                               BuffString  := '';
                      ...
                      else if {( ssLeft in Shift) and }( ssDouble in Shift)
                               then begin // double click
{
                                        tmpStr := Dialogs.InputBox( 'caption', 'prompt', 'default');

                                        Columns[GetColFromX(x)].Title := tmpStr;
                                        doCapture := false; // do not capture after processing
}
                                    end;
              end;
        //////////
        crPlus
            : begin
                  ...
                  if (Pt.X = FCol) and (Pt.Y = FRow)
                      then begin
                               EnsureVisible(FCol, FRow);
                               if (not FReadOnly) and (not FColumns[FCol].FReadOnly)
                                   then begin
                                            IsEditing := True;
                                            FEdit.ShowEdit(FCol, FRow);
                                        end;
                           end
                      else if (Pt.X <> -1) and (pt.Y <> -1)
                               then begin
        ...
        then begin
                  if (Cursor = crPlus)
                      then begin
                               ...
                               else if (Cursor = crRight)
                                        then begin
                                                 i := GetRowFromY(Y);

                                                 if (i <> -1) and (i <> LastHover)
                                                     then begin
                                                              LastHover := i;
                                                              t := Min(i, FRow);
                                                              b := Max(i, FRow);
                                                 ...
                                                 else if (Cursor = crHSplit)
                                                          then begin
                                                                   Suggested := Max(5, X + FHorzOffset - SizingColX - FixedWidth);

                                                                   if FFitToWidth
                                                                       then begin
                                                                                if (SizingCol = ColCount-1) or (SizingCol = -1)
                                                                                    then begin
                                                                                             inherited;
                                                                                             Exit;
                                                                                         end;
                                                                                ...
                                                                                if (Columns[SizingCol+1].FWidth < 5)
                                                                                    then begin
...
begin
    ...
    msg.WheelDelta := msg.WheelDelta div 5; // normalized mouse wheel
...
begin
    if (FWidth <> Value)
        then begin
                 ...
                 // revalidate columns
                 GetGrid.Recalculate; // fix. scrollbar did not show up after adding or changing columns
             end;
end;
...
begin
    ...
    Result := AddItem( FGrid.CreateColumn, index); // fix
end;
