...
/////////////////////////////////////
/// 2012.june.24
/// modifications by Pavel Bugaevskiy, inline-coder.net, inlinecoder@gmail.com
/// some code improvements and simplifying
/// cross-platform removed! freezes were removed. All is working great now.
}
...
procedure TSynURIOpener.NewMouseUp(Sender: TObject; Button: TMouseButton; Shift: TShiftState; X, Y: Integer);
var ptLineCol  : TBufferCoord;
    TokenType,
    Start      : Integer;
    Token      : UnicodeString;
    Attri      : TSynHighlighterAttributes;
begin
    if    (Button <> mbLeft)
       or (         FCtrlActivatesLinks
            and not FControlDown
          )
       or ( Abs(FMouseDownX - X) > 4)
       or ( Abs(FMouseDownY - Y) > 4)
        then exit;

    with TAccessCustomSynEdit(FEditor) do
        begin
            if     ( eoDragDropEditing in Options)
               and ( IsPointInSelection(ptLineCol))
                then exit;
            ...
            if not(
                  Assigned(URIHighlighter)
               and (
                        ( Attri = URIHighlighter.URIAttri)
                     or ( Attri = URIHighlighter.VisitedURIAttri)
                   )
               and not
                   (
                         (eoDragDropEditing in Options)
                     and IsPointInSelection(ptLineCol)
                   )
              )
                 then exit;
...
begin
    ...
    ShellExecute( 0, nil, pchar( URI), nil, nil, SW_SHOWNORMAL);
end;
