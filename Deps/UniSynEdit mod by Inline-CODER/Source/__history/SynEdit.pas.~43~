...
{$ENDIF}
  ...
  public
    ...
    // dramatically improves loading\setting the strings ( synedit.lines.text := ...   -> synedit.replaceLines(*); )
    procedure replaceLines( _l : TSynEditStringList);
...
begin
  ...
  // fRightEdge has to be set before FontChanged is called for the first time
    ...
    if fFontDummy.Name = 'adobe-courier'
        then fFontDummy.Size := 12
        else begin
...
procedure TCustomSynEdit.replaceLines( _l : TSynEditStringList);
var oldL : TStrings;
begin
    oldL := fLines;
    oldL.Clear();
    ...
    /////////////
    //LinesChanging( self);
    ListCleared( self); // force this to take immediate changings
    UpdateScrollBars();
    //LinesChanged( self);
end;
...
{$ENDIF}
begin
    if (PaintLock <> 0) or not (Focused or FAlwaysShowCaret)
        then Include(fStateFlags, sfCaretChanged)
        else begin
                 Exclude(fStateFlags, sfCaretChanged);
                 vCaretDisplay := DisplayXY;

                 if WordWrap and (vCaretDisplay.Column > CharsInWindow + 1)
                     then vCaretDisplay.Column := CharsInWindow + 1;
                 ...
                 if      (CX >= iClientRect.Left)
                     and (CX < iClientRect.Right)
                     and (CY >= iClientRect.Top)
                     and (CY < iClientRect.Bottom)
                     then begin
                              SetCaretPos(CX, CY);
                              ShowCaret;
                          end
...
function TCustomSynEdit.DoMouseWheel(Shift: TShiftState; WheelDelta: Integer; MousePos: TPoint): Boolean;
const WHEEL_DIVISOR = 120; // Mouse Wheel standard
var iWheelClicks   : Integer;
...
begin
{
  ...
  Result := True;
    ...
    if GetKeyState(VK_CONTROL) < 0
        then iLinesToScroll := LinesInWindow shr Ord(eoHalfPageScroll in fOptions)
        else iLinesToScroll := 5;
    ...
    for ndx := 1 to iLinesToScroll do
        begin
            TopLine := TopLine - iWheelClicks;
            Update();
            Sleep( 16);
        end;
...
procedure TCustomSynEdit.ListPutted(Sender: TObject; Index: Integer; aCount: Integer);
var L       : Integer;
   vEndLine : Integer;
begin
  vEndLine := Index +1;
...
begin
  ...
  Exclude(fStateFlags, sfCaretVisible);
    ...
    if Focused or FAlwaysShowCaret
        then begin
                 {$IFDEF SYN_CLX} CreateCaret(self, 0, cw, ch); {$ELSE}
                   ...
                   try
                       if SelAvail
                           then begin
                                    ...
                                    iUndoEnd := fBlockEnd;
                                    SetSelTextPrimitive('');
                                    fUndoList.AddChange(crDelete, iUndoBegin, iUndoEnd, Helper, fActiveSelectionMode);
                                end;
                       Temp := LineText;
                       Temp2 := Temp;
                       ...
                       if Len > 0
                           then begin
                                        ...
                                        then begin
                                                 if CaretX > 1
                                                     then begin
                                                              ...
                                                              ProperSetLine(CaretY - 1, Temp);
                                                              fUndoList.AddChange(crLineBreak, CaretXY, CaretXY, Temp2, smNormal);

                                                              if Command = ecLineBreak
                                                                  then InternalCaretXY := BufferCoord( Length(GetLeftSpacing(SpaceCount1,true)) + 1, CaretY + 1);
                                                          end
                                                     else begin
                                                              Lines.Insert(CaretY - 1, '');
                                                              fUndoList.AddChange(crLineBreak, CaretXY, CaretXY, Temp2, smNormal);
                                                              if Command = ecLineBreak
                                                                  then InternalCaretY := CaretY + 1;
                                                          end;
                                             end
                                        else begin
                                                 SpaceCount2 := 0;
                                                 BackCounter := CaretY;
                                                 if eoAutoIndent in Options
                                                     then begin
                                                              repeat
                                                                  Dec(BackCounter);
                                                              ...
                                                              if SpaceCount2 > 0
                                                                  then begin
                                                                           ...
                                                                           for i := 1 to Length(SpaceBuffer) do
                                                                               if SpaceBuffer[i] = #9
                                                                                   then CommandProcessor(ecTab, #0, nil)
                                                                                   else CommandProcessor(ecChar, SpaceBuffer[i], nil);
                                                                       end;
                                                          end;
                                    ...
                                    if Command = ecLineBreak
                                        then InternalCaretX := SpaceCount2 + 1;
...
begin
    if not Assigned(fSearchEngine)
        then raise ESynEditError.Create('No search engine has been assigned');
    ...
    if not SelAvail
        then Exclude(AOptions, ssoSelectedOnly);

    if (ssoSelectedOnly in AOptions)
        then begin
                 ptStart := BlockBegin;
                 ptEnd := BlockEnd;
                 ...
                 if (fActiveSelectionMode = smLine)
                     ...
                     else if (fActiveSelectionMode = smColumn)
                              then // make sure the start column is smaller than the end column
                                   if (ptStart.Char > ptEnd.Char)
                                       then SwapInt(Integer(ptStart.Char), Integer(ptEnd.Char));

                 // ignore the cursor position when searching in the selection
                 if bBackward
                     then ptCurrent := ptEnd
                     else ptCurrent := ptStart;
             end
        else begin
                 ptStart.Char := 1;
                 ptStart.Line := 1;
    ...
    if bReplaceAll and not bPrompt
        then begin
                 IncPaintLock;
                 BeginUndoBlock;
                 bEndUndoBlock := True;
             end
        else bEndUndoBlock := False;
    ...
    try
        while (ptCurrent.Line >= ptStart.Line) and (ptCurrent.Line <= ptEnd.Line) do
            begin
                ...
                if bBackward
                    then n := Pred(fSearchEngine.ResultCount)
                    else n := 0;
                ...
                while nInLine > 0 do
                    begin
                       ...
                       if not InValidSearchRange(nFound, nFound + nSearchLen)
                           then continue;
                       ...
                       if bBackward
                           then InternalCaretXY := BlockBegin
                           else InternalCaretXY := ptCurrent;

                       // If it's a search only we can leave the procedure now.
                       if not (bReplace or bReplaceAll)
                           then exit;

                       // Prompt and replace or replace all.  If user chooses to replace
                       // all after prompting, turn off prompting.
                       if bPrompt and Assigned(fOnReplaceText)
                           then begin
                                    ...
                                    // added by Inline-CODER.
                                    // I think this will be a right behavior
                                    if nAction = raSkip
                                        then begin
                                                 // move caret X++
...
begin
    ...
    if (eoTabIndent in Options) and ((SelTabBlock) or (OldSelTabLine))
        then begin
                 DoBlockIndent;
...
{ // comments due to unobvious behavior
    if eoSmartTabs in fOptions
        then begin
                 iLine := CaretY - 1;

                 if (iLine > 0) and (iLine < Lines.Count)
                     then begin
                              Dec(iLine);
                              repeat
                                  //todo: rethink it
    ...
    try
        ...
        if eoTabsToSpaces in fOptions
            ...
            else if      (eoTrimTrailingSpaces in Options)
                     and (StartOfBlock.Char > Length(LineText))
                     then begin
                              ...
                              if (eoSmartTabs in fOptions) and not vIgnoreSmartTabs and (iLine > -1)
                                  ...
                                  else nDistanceToTab := TabWidth - ((nPhysX - 1) mod TabWidth);
                                  ...
                                  then begin
                                           ...
                                           if Spaces = ''
                                               then begin
                                                        ...
                                                        if nDistanceToTab > 0
                                                            then Spaces := Spaces + UnicodeStringOfChar(#32, nDistanceToTab);
                                                    end
                                               else Spaces := #9;
                                       end
                                  else begin
                              ...
                              if (eoTrimTrailingSpaces in Options) and (Length(TrimTrailingSpaces(LineText)) = 0)
                                  then NewCaretX := StartOfBlock.Char + GetExpandedLength(Spaces, TabWidth)
                                  else NewCaretX := StartOfBlock.Char + Length(Spaces);
                         end;
        ...
        if fActiveSelectionMode <> smColumn
            then begin
                     fUndoList.AddChange(crInsert, StartOfBlock, CaretXY, SelText, fActiveSelectionMode);
                 end;
    finally
        fUndoList.EndBlock;
    end;
    ...
    try
        ...
        if ChangeScroll
            then Exclude(fOptions, eoScrollPastEol);
    end;
