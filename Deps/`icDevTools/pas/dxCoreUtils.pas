...
uses
  Windows, Graphics, SysUtils, Classes, Controls, TypInfo, dxCore;
...
begin
    if (Flags and DT_CALCRECT <> 0) and ((AText = '') or AShowAccelChar and (AText[1] = '&') and (AText[2] = #0))
        then AText := AText + ' ';

    if not AShowAccelChar
    ...
    with ACanvas do
        begin
            Font.Assign( AFont);
            if not AEnabled
                then Font.Color := dxColor_Msc_Dis_Caption_WXP;
