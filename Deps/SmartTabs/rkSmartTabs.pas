...
type
  ...
  TWMMouseWheel = record
    ...
    TEvent_OnSetActiveTab = procedure ( _Sender : TObject; _index : Integer) of object;
    ...
    TrkSmartTabs =
        class( TCustomControl)
            private
                ...
                //
                FEvent_OnSetActiveTab : TEvent_OnSetActiveTab;
                //
                ...
                procedure DoMouseWheel(var Msg: TWMMouseWheel); reintroduce; message WM_MOUSEWHEEL;
            ...
            protected
                ...
                BmTWidth: Integer;
                BmTabs: TRect;
                Brushes: array [0 .. 8] of TGPSolidBrush;
                CMP: TPoint;
            ...
            public
                ...
                procedure AddObject_focus( _title : String; _object : TObject; _focus : Boolean = true); // focus or not
  ...
  published
    ...
    //
    property OnSetActiveTab : TEvent_OnSetActiveTab read FEvent_OnSetActiveTab write FEvent_OnSetActiveTab;
  end;
...
begin
    AddObject_focus( ATab, AObject);
end;

procedure TrkSmartTabs.AddObject_focus( _title : String; _object : TObject; _focus : Boolean = true);
begin
    if _object <> nil
        then Tabs.AddObject( _title, _object)
        else Tabs.Add      ( _title);

    if _focus
        then FActiveTab := Tabs.Count - 1;

    if (ScrollMode) and (ActiveTab >= FPinnedTabs)
        then begin
                 CalcTabWidth;
                 ScrollOff := 0;
...
//    if Assigned(FOnTabChange) and (not InCreate)
//        then FOnTabChange(Self);
    if _focus
        then if Assigned( FEvent_OnSetActiveTab) and ( not InCreate)
                 then FEvent_OnSetActiveTab( self, FActiveTab);
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      ScrollMax := (((ScrollTabs * (FTabWidth - FTabOverLap)) + FTabOverLap) - BmTWidth);
    end;
  end;
...
begin
  ...
  Brushes[8] := TGPSolidBrush.Create(MakeColor(255, 0, 0, 0));
...
begin
  ...
  ScrollTimer.Interval := 16; //100;// 25;
end;

procedure TrkSmartTabs.DeleteTab(AIndex: Integer);
begin
    if not ((AIndex >= 0) and (AIndex < Tabs.Count))
        then exit;


    Tabs.Delete(AIndex);

    if (FActiveTab > Tabs.Count - 1)
        then FActiveTab := Tabs.Count - 1;

    //if Assigned(FOnTabChange)
    //    then FOnTabChange(Self);
    if Assigned( FEvent_OnSetActiveTab)
        then Fevent_onSetActiveTab( self, FActiveTab);
...
begin
  ...
  begin
    if (x >= BmPWidth + 20) and (x <= Width - 44) then
    begin
  ...
  begin
    ...
    if (i > 0) then
    begin
      if (i = FPinnedTabs) and (ScrollMode) then
        InFrontOf := True
      else
      begin
        Tab := GetTabPoly(i - 1);
...
begin
  if Idx = -1 then
    Exit;
  n := (Idx * (FTabWidth - FTabOverLap));
  if n < ScrollOff then
    ScrollOff := (Idx * (FTabWidth - FTabOverLap))
  else if (n + FTabWidth) > ScrollView.y then
    ScrollOff := ScrollOff + ((n + FTabWidth) - ScrollView.y);
  if ScrollOff > ScrollMax then
    ScrollOff := ScrollMax;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (i <> -1) then
        begin
          ActiveTab := i; // аффтор повний даун, тут вже визивається FOnTabChange, а він хоче визивать його ще раз.
          //if Assigned(FOnTabChange) then
          //  FOnTabChange(Self);
          Invalidate;
          FDragTab := i;
...
begin
  ...
  begin
    ...
    if (FDragging) and (FDragTab >= FPinnedTabs) then
    begin
      y := Height shr 1;
      i := GetTabAtXY(x, y);
      if (i <> FDragTab) and (i <> -1) then
...
begin
  ...
  begin
    ...
    FDragOff:= 0;
    if ScrollMode then
      ActiveTab:= ActiveTab //SetInView(FActiveTab, 0)
    else
      Invalidate;
  end;
...
begin
  ...
  begin
    ...
    if FPinnedTabs > 0 then
    begin
      x1 := (FTabOffset + BmPWidth) - PinnedWidth;
      x2 := x1 + PinnedWidth;
      bx := (x1 + PinnedWidth);
      ax := -1;
    ...
    if ScrollOff = 0 then
      TabsCanvas.FillPolygon(Brushes[0], PGPPoint(@TabAdd), 8)
    else
      TabsCanvas.FillPolygon(Brushes[2], PGPPoint(@TabAdd), 8);
    TabsCanvas.DrawPolygon(baGP, PGPPoint(@TabAdd), 8);
    p := p + 20;

    // t1 = FirstTab t2 = LastTab we gone render
    t1 := ScrollView.x div (FTabWidth - FTabOverLap);
    if t1 > 0 then
      t1 := t1 - 1;
    if t1 < 0 then
      t1 := 0;
    t2 := ScrollView.y div (FTabWidth - FTabOverLap);
    if t2 > ScrollTabs - 1 then
      t2 := ScrollTabs - 1;
    ScrollBmp := TBitmap.Create;
    ScrollBmp.PixelFormat := pf32Bit;
    ScrollBmp.Width := (((t2 - t1) + 1) * (FTabWidth - FTabOverLap)) + FTabOverLap + 1;
    ...
    i := ScrollOff - (t1 * (FTabWidth - FTabOverLap));
    bo := BmTabs.Left - i;
    ...
    y2 := Height - 1;
    by := y1 + 4;
    x1 := (ScrollBmp.Width - FTabWidth) - 1;
    x2 := x1 + FTabWidth;
    bx := (x1 + FTabWidth);
    ClsBtnX := bx;
    bClose := False;
    ...
    begin
      ...
      begin
        ...
        if i + FPinnedTabs = FHotIdx then
          ...
          h := 0;
        ScrollCanvas.FillPolygon(Brushes[h], PGPPoint(@Tab), PolyTabCount);
        ScrollCanvas.DrawPolygon(Pens[h], PGPPoint(@Tab), PolyTabCount);
        ...
        begin
          ...
          begin
            ...
            Row2 := PRGBAArray(slPtr2);
            Row3 := PRGBAArray(slPtr3);
            for w := 0 to FTabOverLap - 2 do // rmk
              if Row2[w + (Mask.Width - FTabOverLap)].a <> 0 then
              begin
        ...
        begin
          ...
          ScrollCanvas.DrawString(PChar(txt), Length(txt), TabsFont, TxtRect, TxtFormat,
            TabsTxtBrush);
          TabsFont.Free;
          TxtFormat.Free;
          TabsTxtBrush.Free;
        ...
        if ScrollBmp.Canvas.TextWidth(txt) > (TabR.Right - TabR.Left) then
        begin
          ...
          // bug! Exception can be raisen when many tabs are added at time
          /////////////////
    ...
    begin
      ...
      begin
        ...
        if x1 + tw > ScrollBmp.Width - 1 then
      ...
      txt := FTabs[FActiveTab];
      Tab := GetTabsPoly(x1, y1, x2, y2);
      ScrollCanvas.FillPolygon(Brushes[2], PGPPoint(@Tab), PolyTabCount);
      ScrollCanvas.DrawPolygon(Pens[2], PGPPoint(@Tab), PolyTabCount);

      ScrollBmp.Canvas.Pen.Color := clGray;
      ScrollBmp.Canvas.MoveTo(0, y2);
      ScrollBmp.Canvas.LineTo(ScrollBmp.Width, y2);
      ScrollBmp.Canvas.Pen.Color := Color;
      ScrollBmp.Canvas.MoveTo(x1 + 1, y2);
      ScrollBmp.Canvas.LineTo(x2, y2);

      Row := PRGBAArray(Integer(ScrollBmp.ScanLine[Height - 1]));
      for w := 0 to FBmp.Width - 1 do
        Row[w].a := 255;
      Row2 := PRGBAArray(Integer(Back.ScanLine[Height - 1]));
      a := FLevelTabActive;
      c1 := ColorToRGB(FColorTabActive);
      r := Byte(c1);
      g := Byte(c1 shr 8);
      ...
      begin
        ...
        begin
          if FCloseDown then
            ScrollCanvas.FillEllipse(Brushes[4], n - 3, m - 9, 12, 12)
          else
            ScrollCanvas.FillEllipse(Brushes[3], n - 3, m - 9, 12, 12);
          ScrollCanvas.DrawLine(Pens[4], n, m, n + 6, m - 6);
          ScrollCanvas.DrawLine(Pens[4], n + 6, m, n, m - 6);
        end
        else
        begin
          ScrollCanvas.DrawLine(Pens[3], n, m, n + 6, m - 6);
          ScrollCanvas.DrawLine(Pens[3], n + 6, m, n, m - 6);
        end;
...
begin
  ...
  // single button
      if bClose
          then begin
                   n := x2 - 24;
                   m := FTabOffsetTop + (FTabsHeight shr 1) + 4;
                   if (FTabX = i) and FHotClose
                       then begin
                                if FCloseDown
                                    then TabsCanvas.FillEllipse(Brushes[4], n - 3, m - 9, 12, 12)
                                    else TabsCanvas.FillEllipse(Brushes[3], n - 3, m - 9, 12, 12);
    ...
    if Pinned
        then bImages := Assigned(FOnGetImageIdx) and Assigned(FImages)
        else bImages := (FShowImages) and Assigned(FOnGetImageIdx) and Assigned(FImages) and ((FTabWidth > 80) or not FImageHide);
    ...
    // use mask image to restore overlaping tab area
    if (not FDragging) and (not FSeeThruTabs)
        then begin
                 slPtr := Integer(FBmp.ScanLine[y1]);
                 slSize := Integer(FBmp.ScanLine[y1 + 1]) - slPtr;
                 slPtr2 := Integer(Mask.ScanLine[y1]);
                 ...
                 for h := y1 to y2 - 1 do
                     begin
                         ...
                         for w := 0 to FTabOverLap - 1 do
                             if Row2[w + (Mask.Width - FTabOverLap) + 1].a <> 0
                                 then begin
                                          Row[w + x3].r := Row3[w + x3].r;
                                          Row[w + x3].g := Row3[w + x3].g;
                                          Row[w + x3].b := Row3[w + x3].b;
  ...
  begin
    ...
    TabR.Left := TabR.Left + 16;
      ...
      // active tab button close
    if bClose
        then begin
                 n := x2 - 24;
                 m := FTabOffsetTop + (FTabsHeight shr 1) + 4;
                 if (FTabX = ActiveTab) and FHotClose
                     then begin
                              if FCloseDown
                                  then TabsCanvas.FillEllipse(Brushes[4], n - 3, m - 9, 12, 12)
                                  else TabsCanvas.FillEllipse(Brushes[3], n - 3, m - 9, 12, 12);
  ...
  begin
    ...
    p2.x := 0;
    p2.y := 12;
    baGB := TGPLinearGradientBrush.Create(p1, p2, MakeColor(48, 128, 128, 128),
      MakeColor(64, 0, 0, 0));
    baGP := TGPPen.Create(baGB);
    baGP.SetWidth(0.5);
    TabsCanvas.DrawPolygon(baGP, PGPPoint(@TabAdd), 13);
...
begin
    if ( Value = ActiveTab) and ( ActiveTab <> 0)
        then exit; // господі, кретин. Елементарне зробить не може.

    if ( Value >= 0) and ( Value < FTabs.Count)
        then begin
                 FActiveTab := Value;
                 ...
                 if (ScrollMode) and (ActiveTab >= FPinnedTabs)
                     then SetInView(FActiveTab - FPinnedTabs, 5)
                     //else Invalidate;
             end;
